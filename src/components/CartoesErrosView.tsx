...
export function CartoesErrosView() {
  const { editais, managedEditais, updateCartoesErros, editCartaoErro, clearCartoesErros, updateCartaoSM2Erro, removeDuplicateCartoesErros } = useEdital();
           ...
           ar.materias.forEach(mat => {
              ...
              (mat.topicos || []).forEach(top => {
                 ...
                 top.subtopicos?.forEach(sub => {
                    ...
                    if (sub.cartoes_erros) {
                        sub.cartoes_erros.forEach(c => {
                           const q = c.pergunta.trim().toLowerCase();
  ...
  const startStudy = (cardsList: any[], force: boolean = false, onlyNew: boolean = false) => {
    ...
    if (toStudy.length === 0) {
       alert(onlyNew ? "Não há flashcards novos para revisar nesta seleção." : "Nenhum flashcard para revisar agora! Você pode fazer a revisão forçada.");
       return;
    }
  ...
  const answerCard = (quality: number) => {
    ...
    updateCartaoSM2Erro(current.editalId, current.areaId, current.materiaId, current.topicoId, current.subtopicoId, current.id, quality);
  ...
  const handleDeleteDeck = (level: 'materia' | 'topico' | 'subtopico', target: any) => {
     ...
     if (safeConfirm(`Tem certeza que deseja apagar TODOS os ${target.cards.length} flashcards deste ${level}?`)) {
        if (level === 'materia') clearCartoesErros(sample.editalId, sample.areaId, sample.materiaId);
        if (level === 'topico') clearCartoesErros(sample.editalId, sample.areaId, sample.materiaId, sample.topicoId);
        if (level === 'subtopico') clearCartoesErros(sample.editalId, sample.areaId, sample.materiaId, sample.topicoId, sample.subtopicoId);
     }
  };
  ...
  const handleOptimizeDeck = (level: 'materia' | 'topico' | 'subtopico', target: any) => {
     if (target.cards.length === 0) return;
     const sample = target.cards[0];
     if (safeConfirm(`Atenção: Esta ação juntará flashcards que tenham a MESMA pergunta (frente) no nível de assunto selecionado (${level}). Seus devidos versos serão combinados e quaisquer duplicatas removidas. Deseja prosseguir?`)) {
        ...
        if (level === 'topico') removeDuplicateCartoesErros(sample.editalId, sample.areaId, sample.materiaId, sample.topicoId);
        if (level === 'subtopico') removeDuplicateCartoesErros(sample.editalId, sample.areaId, sample.materiaId, sample.topicoId, sample.subtopicoId);
     }
  };
  ...
  const saveToDestination = (newCardsData: {pergunta: string, resposta: string, imagemPergunta?: string, imagemResposta?: string, origem?: string, subtopicoTitulo?: string}[]) => {
    ...
    // Assemble final list to send to updateCartoesErros
      ...
      <div className="flex-1 overflow-y-auto w-full max-w-3xl mx-auto p-4 sm:p-6 bg-slate-50 flex flex-col h-full">
         <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
               <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Revisão de Flashcards</span>
               ...
               {isEditingCartao ? (
                 <div className="flex-1 flex flex-col justify-center gap-4 text-left max-w-2xl mx-auto w-full">
                    ...
                    <div className="flex gap-3 justify-end mt-6">
                       ...
                       <button onClick={() => {
                          editCartaoErro(card.editalId, card.areaId, card.materiaId, card.topicoId, card.subtopicoId, card.id, editPergunta, editResposta, editImagemPergunta.trim() || undefined, editImagemResposta.trim() || undefined, editOrigem.trim() || undefined);
  ...
  return (
    <div className="flex-1 overflow-y-auto w-full max-w-5xl mx-auto p-4 sm:p-6 bg-slate-50 relative pb-24 h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 pt-4 shrink-0 gap-4">
         <div className="flex flex-col">
            <h2 className="text-2xl font-display font-bold text-slate-900 mb-1">Flashcards de Erros</h2>
         ...
         {(editais.length > 0 || managedEditais.length > 0) && (
           <button 
             onClick={() => {
               if (safeConfirm("ATENÇÃO EXTREMA: Você está prestes a apagar TODOS os flashcards de TODOS os editais e matérias na sua conta. Esta ação não pode ser desfeita. Tem certeza absoluta?")) {
                 editais.forEach(e => clearCartoesErros(e.id));
                 managedEditais.forEach(e => clearCartoesErros(e.id));
               }
             }}
             ...
             title="Apagar todos os flashcards (Wipe All Flashcards)"
           >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                   ...
                   if (filteredDecks.length === 0) {
                      return (
                         <div className="text-center p-12 bg-white rounded-3xl border border-slate-200 border-dashed text-slate-500 flex flex-col items-center justify-center gap-2">
                            ...
                            <p className="text-xs text-slate-400 max-w-sm">Use as guias "Criar Manual" ou "Importar em Lote" no topo para criar flashcards associando a um Edital e Matéria.</p>
                         </div>
                      );
                   }
                                  ...
                                  <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 border-b border-slate-150">
                                     ...
                                     <div className="flex items-center gap-4 justify-between sm:justify-end shrink-0 ml-10 sm:ml-0">
                                        <div className="flex items-center gap-2">
                                           {mStats.novos > 0 && (
                                              <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-50 text-blue-600 rounded-md border border-blue-100" title="Novos flashcards">
                                                 {mStats.novos} N
                                              </span>
                                           )}
                                           {mStats.revisoes > 0 ? (
                                        ...
                                        <div className="flex items-center gap-1.5">
                                           ...
                                           <button onClick={() => handleDeleteDeck('materia', m)} className="px-2 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-500 rounded-lg transition-all cursor-pointer mr-0.5" title="Apagar todos flashcards desta matéria"><Trash2 className="w-3.5 h-3.5" /></button>
                                           <button 
                                           ...
                                           {mStats.novos > 0 && (
                                               <button 
                                                  ...
                                                  title="Revisar apenas os flashcards que nunca foram vistos"
                                               >
                                                    ...
                                                    <div className="p-3.5 pl-6 sm:pl-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-l-4 border-indigo-500/20 hover:border-indigo-500 transition-all">
                                                       ...
                                                       <div className="flex items-center gap-3 justify-between sm:justify-end shrink-0 ml-8 sm:ml-0">
                                                          ...
                                                          <div className="flex items-center gap-1">
                                                             ...
                                                             <button onClick={() => handleDeleteDeck('topico', t)} className="px-1.5 py-1 bg-white hover:bg-rose-50 text-slate-300 hover:text-rose-500 rounded transition-all cursor-pointer mr-0.5" title="Apagar todos flashcards deste tópico"><Trash2 className="w-3 h-3" /></button>
                                                             <button 
                                                                ...
                                                                <div key={subKey} className="p-3 pl-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group transition-all hover:bg-indigo-50/20">
                                                                   ...
                                                                   <div className="flex items-center gap-3 justify-between sm:justify-end shrink-0 ml-5 sm:ml-0">
                                                                      ...
                                                                      <div className="flex items-center gap-1">
                                                                         ...
                                                                         <button onClick={() => handleDeleteDeck('subtopico', s)} className="px-1.5 py-1 bg-transparent hover:bg-rose-50 text-slate-300 hover:text-rose-500 rounded transition-all cursor-pointer mr-0.5" title="Apagar todos flashcards deste assunto"><Trash2 className="w-3 h-3" /></button>
                                                                         <button 
             ...
             <div className="flex flex-col gap-6">
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                   ...
                   <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6">Flashcards Totais</p>
           ...
           <div className="flex flex-col gap-6">
              ...
              {activeTab === 'manual' && (
                 <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col gap-4 max-w-2xl">
                    <h3 className="font-bold text-slate-900 mb-2">2. Digite os Dados do Flashcard</h3>
                    <div>
                    ...
                    <button onClick={handleSaveManual} className="mt-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm rounded-xl transition-all uppercase tracking-widest self-end">Adicionar Flashcard</button>
                 </div>
              )}
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative">
                    {conflictMode && (
                        <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-10 p-8 flex flex-col rounded-[2rem] border border-slate-100 shadow-2xl">
                           ...
                           <p className="text-slate-600 mt-2 mb-6 text-sm ml-13">Os flashcards foram importados com sucesso. Notamos que as seguintes perguntas já existiam e foram inseridas novamente no seu banco:</p>
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex-1 flex flex-col h-[60vh]">
                       <h3 className="font-bold text-slate-900 mb-2 mt-2 pt-2 flex justify-between items-center shrink-0">
                          ...
                          {parsedCards.length > 0 && <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-md">{parsedCards.length} flashcards</span>}
                       </h3>
                       <div className="flex-1 overflow-y-auto mb-4 border border-slate-200 bg-slate-50 rounded-2xl p-2 hide-scrollbar">
                          {parsedCards.length === 0 ? (
                             <div className="h-full flex items-center justify-center text-slate-400 text-xs text-center p-4">Gere os flashcards à esquerda para visualizar aqui</div>
                          ) : (
                             <div className="flex flex-col gap-3">
                       ...
                       <button onClick={handleSaveImport} disabled={parsedCards.length === 0 || !selectedTopico} className="w-full px-6 py-3 bg-blue-900 hover:bg-blue-800 disabled:opacity-50 text-white font-bold text-sm rounded-xl transition-all shadow-lg shadow-blue-900/20 uppercase tracking-widest shrink-0">Salvar Flashcards</button>
                    </div>
                 </div>
              )}
           </div>
